...
if not is_client():
    ...
    class SampleRenderer:
        def __init__(self, world_up=(0, 0, 1)):
            ...
            self._world_up = wu / (np.linalg.norm(wu) + 1e-9)
            ...
            self._t_last = 0.0
            colors = np.array([[255, 255, 255], [216, 216, 216], [176, 176, 176]], dtype=np.uint8)
            self._disc = [self._make_disc(c) for c in colors]
        ...
        def _make_disc(base_bgr, n=256):
            ...
            fres = (1 - z) ** 4
            sun = np.array([240, 245, 255], dtype=np.float32)
        ...
        def _sky(self, W, H, view, proj):
            ...
            if self._sky_cache is None or self._sky_cache[0] != (W2, H2):
                ...
                top = np.array([217, 140, 89], dtype=np.float32)
                bot = np.array([173, 158, 153], dtype=np.float32)
            ...
            t = ((vert / nrm).clip(-1, 1) + 1) * 0.5
            t = t * t * (3 - 2 * t)
            return cv2.resize(lut[(t * 255).astype(np.uint8)], (W, H), interpolation=cv2.INTER_LINEAR)

        def _step(self, sim_time):
            # RK4 three-body integration with softening; advances to sim_time at half real-time.
            def accel(p):
                d = p[None] - p[:, None]
                r2 = (d * d).sum(2) + 0.08**2
                np.fill_diagonal(r2, 1)
                f = 0.5 * self._body_mass[None] / (np.sqrt(r2) * r2)
        ...
        def _project_segments(a, b, v2c, eps=0.01):
            ...
            if ca.any():
                t = ((eps - pa[ca, 3]) / (pb[ca, 3] - pa[ca, 3]))[:, None]
                pa[ca] += t * (pb[ca] - pa[ca])
            if cb.any():
                t = ((eps - pb[cb, 3]) / (pa[cb, 3] - pb[cb, 3]))[:, None]
                pb[cb] += t * (pa[cb] - pb[cb])
        ...
        def render(self, cam, sim_time=0):
            ...
            # Row-vector world-to-clip; bypasses HoloViewer's mixed-convention cam["full"].
            v2c = view.T @ proj
            ...
            nda, ndb, mask = self._project_segments(
                np.r_[np.c_[g, lo, z0], np.c_[lo, g, z0]],
...
class CubeSampleViewer(HoloViewer):
    def __init__(self, **kw):
        super().__init__(window_name="HoloViewer Three-Body Sample",
                         axis_mode="Z_UP", show_axes=True, **kw)
        self._r = None
